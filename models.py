...
class SimpleAutoencoder(nn.Module):
    ...
    def __init__(self, input_dim=24, latent_dim=3, hidden_dim=16, dropout=0.1, n_hidden_layers=2, hidden_dims=None):
        """
        ...
        # If explicit layer list provided, use it; otherwise fall back to single repeated hidden_dim
        if hidden_dims is not None and len(hidden_dims) > 0:
            self.hidden_dims = hidden_dims
            self.n_hidden_layers = len(hidden_dims)
        else:
            self.hidden_dims = [hidden_dim] * max(1, n_hidden_layers)
        ...
        # Build encoder layers dynamically based on hidden_dims list
        encoder_layers = []
        prev_dim = input_dim
        for hdim in self.hidden_dims:
            encoder_layers.extend([
                nn.Linear(prev_dim, hdim),
                nn.LayerNorm(hdim, eps=1e-6, elementwise_affine=True),
                nn.ReLU(),
                nn.Dropout(dropout)
            ])
            prev_dim = hdim
        ...
        decoder_layers = []
        prev_dim = latent_dim
        for hdim in reversed(self.hidden_dims):
...
def create_dynamic_autoencoder(input_dim: int = 24, latent_dim: int = 3, 
                              hidden_dims: List[int] | None = None, dropout: float = 0.1) -> DynamicAutoencoder:
    """
