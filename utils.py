...
def plot_latent_encodings(model, dataloader, device: str, latent_dim: int, 
    ...
    """
    Generate 2D scatter plot of latent space encodings if latent_dim == 2.
    ...
    Returns:
        Path to saved plot file, or None if latent_dim != 2
    """
    if latent_dim != 2:
        return None
    ...
    try:
        ...
        plt.scatter(all_latents[:, 0], all_latents[:, 1], 
                   ...
                   label=f'Training Samples ({len(all_latents)} total)')
        ...
        # Add statistics
        stats_text = (f'Total Samples: {len(all_latents)}\\n'
                     f'Range X: [{all_latents[:, 0].min():.3f}, {all_latents[:, 0].max():.3f}]\\n'
...
def create_combined_plot(epochs: np.ndarray, losses: np.ndarray, current_epoch: int,
                        ...
                        title_suffix: str = "") -> Tuple[Path, Optional[Path]]:
    """
    ...
    # Only create 2D latent space subplot if latent_dim is exactly 2
    if (latent_dim == 2 and model is not None and dataloader is not None):
        # Create combined figure with subplots for 2D latent space
        ...
        # Right subplot: 2D Latent space visualization
